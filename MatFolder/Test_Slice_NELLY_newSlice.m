...
% NELLY TEST DATA z=83 

cd 'CST_Data\Nelly_newSlice'

T_Matrix = xlsread('Nelly_z83.xlsx');
files = dir('*.txt');
...
coordSensors = [0 65; 0 -75; -59 -7; 56 -7]; 

%4 Sensors: Temperatures
tempSensors = [38.2; 38.3; 38.1; 38.1]; 
    ...
    for s=1:N_s
        for p=1:N_p
           ...
           is_in_CSF = inpolygon(dx(i,s,p),dy(i,s,p),CSF1(:,1),CSF1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF2(:,1),CSF2(:,2)) || ...
                              ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF85(:,1),CSF85(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF86(:,1),CSF86(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF87(:,1),CSF87(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF88(:,1),CSF88(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF89(:,1),CSF89(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF90(:,1),CSF90(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF91(:,1),CSF91(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF92(:,1),CSF92(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF93(:,1),CSF93(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF94(:,1),CSF94(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF95(:,1),CSF95(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF96(:,1),CSF96(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF97(:,1),CSF97(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF98(:,1),CSF98(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF99(:,1),CSF99(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF100(:,1),CSF100(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF101(:,1),CSF101(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF102(:,1),CSF102(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF103(:,1),CSF103(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF104(:,1),CSF104(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF105(:,1),CSF105(:,2)) ; 
           if is_in_CSF
            ...
            is_in_GreyMatter = inpolygon(dx(i,s,p),dy(i,s,p),GM_innerWM1(:,1),GM_innerWM1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),GM_innerWM2(:,1),GM_innerWM2(:,2)) ;
            if is_in_GreyMatter
            ...
            is_in_WhiteMatter = inpolygon(dx(i,s,p),dy(i,s,p),WM1(:,1),WM1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),WM2(:,1),WM2(:,2)) || ...
                                ...
                                inpolygon(dx(i,s,p),dy(i,s,p),WM7(:,1),WM7(:,2));
            ...
            is_in_Muscle = inpolygon(dx(i,s,p),dy(i,s,p),Muscle1(:,1),Muscle1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Muscle2(:,1),Muscle2(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Muscle_Contour(:,1),Muscle_Contour(:,2))  ;
            if is_in_Muscle
...
t_Data_z83_N = [ptesDiel(:,:),normDistance(:,:),tempSensors(:)];  %Data Matrix: Columns = dielectric properties (epsilon, sigma, rho), distance to Sensors, temperature at Sensor, Temperature at Point P itself. 
...
t=uitable('parent',f,'data',t_Data_z83_N,'columnname',h,'columnwidth',{54},'position',[5 10 995 640]);


M_z83_N = [X, Y, T]; 

clearvars -except x_Data M_Train t_Data_z60 M_z60 t_Data_z50 M_z50 t_Data_z90_N M_z90_N t_Data_z90 M_z90 t_Data_z80 M_z80 t_Data_z70_N M_z70_N t_Data_z60_N M_z60_N t_Data_z83_N M_z83_N

save ('MatFolder\Test_NELLYz83.mat', 't_Data_z83_N'); 
