...
% NELLY TEST DATA z=70 

cd 'CST_Data\Test_NELLY_z70'

T_Matrix = xlsread('Nelly_TEST_z70.xlsx');
files = dir('*.txt');
...
coordSensors = [0 55.6; 0 -70; -57.3 -14.0; 53 -14.0]; 

%4 Sensors: Temperatures
tempSensors = [38.1; 38.4; 38.4; 38.4]; 
    ...
    for s=1:N_s
        for p=1:N_p
           ...
           is_in_CSF = inpolygon(dx(i,s,p),dy(i,s,p),CSF1(:,1),CSF1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF2(:,1),CSF2(:,2)) || ...
                              ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF55(:,1),CSF55(:,2)) ; 
           if is_in_CSF
            ...
            is_in_Muscle = inpolygon(dx(i,s,p),dy(i,s,p),Muscle(:,1),Muscle(:,2)) ;
            if is_in_Muscle
...
t_Data_z70_N = [ptesDiel(:,:),normDistance(:,:),tempSensors(:)];  %Data Matrix: Columns = dielectric properties (epsilon, sigma, rho), distance to Sensors, temperature at Sensor, Temperature at Point P itself. 
...
t=uitable('parent',f,'data',t_Data_z70_N,'columnname',h,'columnwidth',{54},'position',[5 10 995 640]);


M_z70_N = [X, Y, T]; 

clearvars -except x_Data M_Train t_Data_z60 M_z60 t_Data_z50 M_z50 t_Data_z90_N M_z90_N t_Data_z90 M_z90 t_Data_z80 M_z80 t_Data_z70_N M_z70_N

save ('MatFolder\Test_NELLYz70.mat', 't_Data_z70_N'); 
