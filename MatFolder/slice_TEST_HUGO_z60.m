...
cd 'CST_Data\Test_HUGO_z60';

T_Matrix = xlsread('HUGO_Test_z60.xlsx');
files = dir('*.txt');
...
coordSensors = [0 66; 0 -70; -49 5; 51 5]; 

%4 Sensors: Temperatures
tempSensors = [38.6; 38.2; 38.7; 38]; 
    ...
    for s=1:N_s
        ...
        is_in_Air = inpolygon(dx(i,s,p),dy(i,s,p),Air(:,1),Air(:,2));
         ...
         is_in_Blood = inpolygon(dx(i,s,p),dy(i,s,p),Blood1(:,1),Blood1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Blood2(:,1),Blood2(:,2)) || ...
                       ...
                       inpolygon(dx(i,s,p),dy(i,s,p),Blood7(:,1),Blood7(:,2)) ;
            if is_in_Blood
           ...
           is_in_CSF = inpolygon(dx(i,s,p),dy(i,s,p),CSF1(:,1),CSF1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF2(:,1),CSF2(:,2)) || ...
                              ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF43(:,1),CSF43(:,2)) ;
           if is_in_CSF
           ...
           is_in_GreyMatter = inpolygon(dx(i,s,p),dy(i,s,p),GM1(:,1),GM1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),GM2(:,1),GM2(:,2)) || ...
            ...
            is_in_WhiteMatter = inpolygon(dx(i,s,p),dy(i,s,p),WM1(:,1),WM1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),WM2(:,1),WM2(:,2)) || ...
                              ...
                              inpolygon(dx(i,s,p),dy(i,s,p),WM9(:,1),WM9(:,2)) ; 
            ...
            is_in_GreyMatter2 = inpolygon(dx(i,s,p),dy(i,s,p),GM_outer(:,1),GM_outer(:,2)) ;
...
t_Data_z60 = [ptesDiel(:,:),normDistance(:,:),tempSensors(:)];  %Data Matrix: Columns = dielectric properties (epsilon, sigma, rho), distance to Sensors, temperature at Sensor, Temperature at Point P itself. 
...
t=uitable('parent',f,'data',t_Data_z60,'columnname',h,'columnwidth',{54},'position',[5 10 995 640]);
...
cd 'C:\Users\Julie\Desktop\NITE_deuxieme\NITE\Data\2019-04-24\Train_Slice2_z60';

T_Matrix_z60 = xlsread('HUGO_Train_z60_v2.xlsx');
files_z60 = dir('*.txt');

for i=1:length(files_z60)
    eval(['load ' files_z60(i).name ' -ascii']);
end

X_z60=T_Matrix_z60(:,1);
Y_z60=T_Matrix_z60(:,2);
T_z60=T_Matrix_z60(:,3);
format Long

i_select_z60 = inpolygon(X_z60,Y_z60,Skin_Outer(:,1),Skin_Outer(:,2));
X_z60 = X_z60(i_select_z60);
Y_z60 = Y_z60(i_select_z60);
T_z60 = T_z60(i_select_z60);

M_z60 = [X_z60, Y_z60, T_z60]; 


clearvars -except x_Data M_Train t_Data_z60 M_z60

save ('MatFolder\Test_HUGOz60.mat', 't_Data_z60');
