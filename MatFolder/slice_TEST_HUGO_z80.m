...
cd 'CST_Data\Test_HUGO_z80';

T_Matrix = xlsread('HUGO_z80.xlsx');
files = dir('*.txt');
...
coordSensors = [0 78; 0 -83; -58.9 0; 60.9 0]; 

%4 Sensors: Temperatures
tempSensors = [38.3; 38.4; 38.1; 37.8]; 
        ...
        is_in_Air = inpolygon(dx(i,s,p),dy(i,s,p),Air(:,1),Air(:,2));
         ...
         is_in_Blood = inpolygon(dx(i,s,p),dy(i,s,p),Blood1(:,1),Blood1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Blood2(:,1),Blood2(:,2)) || ...
           ...
           is_in_CSF = inpolygon(dx(i,s,p),dy(i,s,p),CSF1(:,1),CSF1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),CSF2(:,1),CSF2(:,2)) || ...
                              ...
                              inpolygon(dx(i,s,p),dy(i,s,p),CSF53(:,1),CSF53(:,2)) ;
           if is_in_CSF
           ...
           is_in_GreyMatter = inpolygon(dx(i,s,p),dy(i,s,p),GM1(:,1),GM1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),GM2(:,1),GM2(:,2)) || ...
            ...
            is_in_WhiteMatter = inpolygon(dx(i,s,p),dy(i,s,p),WM1(:,1),WM1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),WM2(:,1),WM2(:,2)) || ...
                              ...
                              inpolygon(dx(i,s,p),dy(i,s,p),WM9(:,1),WM9(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),WM10(:,1),WM10(:,2));
            ...
            is_in_Bone = inpolygon(dx(i,s,p),dy(i,s,p),Bone(:,1),Bone(:,2));
            if is_in_Bone
               ...
               is_in_Muscle = inpolygon(dx(i,s,p),dy(i,s,p),Muscle1(:,1),Muscle1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Muscle2(:,1),Muscle2(:,2)) || ... 
                              ...
                              inpolygon(dx(i,s,p),dy(i,s,p),Muscle13(:,1),Muscle13(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Muscle14(:,1),Muscle14(:,2)) ;
...
t_Data_z80 = [ptesDiel(:,:),normDistance(:,:),tempSensors(:)];  %Data Matrix: Columns = dielectric properties (epsilon, sigma, rho), distance to Sensors, temperature at Sensor, Temperature at Point P itself. 
...
t=uitable('parent',f,'data',t_Data_z80,'columnname',h,'columnwidth',{54},'position',[5 10 995 640]);
...
cd 'C:\Users\Julie\Desktop\NITE_May2019\Matlab_Code_2\HUGO-z80-july2019';

T_Matrix_z80 = xlsread('HUGO_z80.xlsx');
files_z80 = dir('*.txt');

for i=1:length(files_z80)
    eval(['load ' files_z80(i).name ' -ascii']);
end

X_z80=T_Matrix_z80(:,1);
Y_z80=T_Matrix_z80(:,2);
T_z80=T_Matrix_z80(:,3);
format Long

i_select_z80 = inpolygon(X_z80,Y_z80,Skin_Contour(:,1),Skin_Contour(:,2));
X_z80 = X_z80(i_select_z80);
Y_z80 = Y_z80(i_select_z80);
T_z80 = T_z80(i_select_z80);

M_z80 = [X_z80, Y_z80, T_z80]; 


clearvars -except x_Data M_Train t_Data_z60 M_z60 t_Data_z50 M_z50 t_Data_z90_N M_z90_N t_Data_z90 M_z90 t_Data_z80 M_z80

save ('MatFolder\Test_HUGOz80.mat', 't_Data_z80');
