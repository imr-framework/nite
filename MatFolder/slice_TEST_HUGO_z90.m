...
cd 'CST_Data\Test_HUGO_z90';

T_Matrix = xlsread('HUGO_z90.xlsx');
files = dir('*.txt');
...
coordSensors = [0 -81; 0 -87; -60 0; 62 0]; 

%4 Sensors: Temperatures
tempSensors = [37.9; 37.8; 38.0; 37.9]; 
...
for i=1:N
    for s=1:N_s
        ...
        is_in_Air = inpolygon(dx(i,s,p),dy(i,s,p),Air(:,1),Air(:,2));
             if is_in_Air
         ...
         is_in_Blood = inpolygon(dx(i,s,p),dy(i,s,p),Blood1(:,1),Blood1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Blood2(:,1),Blood2(:,2)) || ...
                       ...
                       inpolygon(dx(i,s,p),dy(i,s,p),Blood13(:,1),Blood13(:,2)) ;
            if is_in_Blood
           ...
           is_in_GreyMatter = inpolygon(dx(i,s,p),dy(i,s,p),GM1(:,1),GM1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),GM2(:,1),GM2(:,2)) || ...
                              ...
                              inpolygon(dx(i,s,p),dy(i,s,p),GM7(:,1),GM7(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),GM8(:,1),GM8(:,2)) || ...
                              inpolygon(dx(i,s,p),dy(i,s,p),GM9(:,1),GM9(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),GM10(:,1),GM10(:,2)) ;
            ...
            is_in_WhiteMatter = inpolygon(dx(i,s,p),dy(i,s,p),WM1(:,1),WM1(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),WM2(:,1),WM2(:,2)) || ...
                              ...
                              inpolygon(dx(i,s,p),dy(i,s,p),WM7(:,1),WM7(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),WM_big(:,1),WM_big(:,2));
            ...
            is_in_Fat = inpolygon(dx(i,s,p),dy(i,s,p),Fat_Contour(:,1),Fat_Contour(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Fat1(:,1),Fat1(:,2)) || ...
                        inpolygon(dx(i,s,p),dy(i,s,p),Fat2(:,1),Fat2(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Fat3(:,1),Fat3(:,2)) || ...
                        inpolygon(dx(i,s,p),dy(i,s,p),Fat4(:,1),Fat4(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Fat3(:,1),Fat3(:,2)) || ...
                        inpolygon(dx(i,s,p),dy(i,s,p),Fat6(:,1),Fat6(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Fat3(:,1),Fat3(:,2)) || ...
                        inpolygon(dx(i,s,p),dy(i,s,p),Fat8(:,1),Fat8(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Fat3(:,1),Fat3(:,2)) || ...
                        inpolygon(dx(i,s,p),dy(i,s,p),Fat10(:,1),Fat10(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Fat3(:,1),Fat3(:,2)) || ...
                        inpolygon(dx(i,s,p),dy(i,s,p),Fat12(:,1),Fat12(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Fat3(:,1),Fat3(:,2)) || ...
                        inpolygon(dx(i,s,p),dy(i,s,p),Fat14(:,1),Fat14(:,2)) || inpolygon(dx(i,s,p),dy(i,s,p),Fat15(:,1),Fat15(:,2)) ;
...
t_Data_z90 = [ptesDiel(:,:),normDistance(:,:),tempSensors(:)];  %Data Matrix: Columns = dielectric properties (epsilon, sigma, rho), distance to Sensors, temperature at Sensor, Temperature at Point P itself. 
...
t=uitable('parent',f,'data',t_Data_z90,'columnname',h,'columnwidth',{54},'position',[5 10 995 640]);
...
cd 'C:\Users\Julie\Desktop\NITE_May2019\Matlab_Code_2\HUGO-z90-july2019';

T_Matrix_z90 = xlsread('HUGO_z90.xlsx');
files_z90 = dir('*.txt');

for i=1:length(files_z90)
    eval(['load ' files_z90(i).name ' -ascii']);
end

X_z90=T_Matrix_z90(:,1);
Y_z90=T_Matrix_z90(:,2);
T_z90=T_Matrix_z90(:,3);
format Long

i_select_z90 = inpolygon(X_z90,Y_z90,Skin_Contour(:,1),Skin_Contour(:,2));
X_z90 = X_z90(i_select_z90);
Y_z90 = Y_z90(i_select_z90);
T_z90 = T_z90(i_select_z90);

M_z90 = [X_z90, Y_z90, T_z90]; 


clearvars -except x_Data M_Train t_Data_z60 M_z60 t_Data_z50 M_z50 t_Data_z90_N M_z90_N t_Data_z90 M_z90

save ('MatFolder\Test_HUGOz90.mat', 't_Data_z90');
